...
```
    ...
    pushd multiple && mvn -U org.jboss.tools.tycho-plugins:target-platform-utils:0.23.3-SNAPSHOT:fix-versions -DtargetFile=locus-multiple.target && rm -f locus-multiple.target locus-multiple.target_update_hints.txt && mv -f locus-multiple.target_fixedVersion.target locus-multiple.target && popd
